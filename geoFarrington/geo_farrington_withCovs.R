...
#' @export
geofarringtonFlexibleWithFormula <- function(sts, formula, covariates, latlong, bw, control = list(
  range = NULL,             # range of time points to be monitored
  b = 5,                    # how many years to go back in time?
  w = 3,                    # half-window length
  is.latlong=FALSE,         # longitude and latitude data?
  ...
  # Fetch observed
  observed <- observed(sts[[1]])
  for(i in 2:length(sts)){
    observed = cbind(observed,observed(sts[[i]]))
  }
  colnames(observed) = paste("observed",c(1:length(sts)),sep="")  
  freq <- sts[[1]]@freq
  if (epochAsDate) {
  ...
  # Fetch population
  population <- population(sts[[1]])
  for(i in 2:length(sts)){
    population = cbind(population,population(sts[[i]]))
  }
  ...
  ######################################################################
  score <- trend <- pvalue <- expected <- qAICs <-
    mu0Vector <- phiVector <- trendVector <-
    matrix(data = 0, nrow = length(control$range), ncol = length(sts))
  ...
  for (k in control$range) {
    ...
    for(i in 1:length(sts)){
      dataGLM_tmp[[i]] <- algo.farrington.data.glm(dayToConsider=dayToConsider,
                                        b=control$b, freq=freq,
                                        epochAsDate=epochAsDate,
                                        epochStr=epochStr,
                                        vectorOfDates=vectorOfDates,w=control$w,
                                        noPeriods=control$noPeriods,
                                        observed=observed[,i],population=population[,i],
                                        verbose=control$verbose,
                                        pastWeeksNotIncluded=control$pastWeeksNotIncluded,k,
                                        covariates)
    }
    dataGLM = data.frame(ID=1,dataGLM_tmp[[1]])
    for(i in 2:length(sts)){
      dataGLM_tmp2 = data.frame(ID=i,dataGLM_tmp[[i]])
      dataGLM = rbind(dataGLM,dataGLM_tmp2)
    }
    
    # calculate weights
    if(control$is.latlong){
      D = distm(latlong[,c("long","lat")])  
    }else{
      D = as.matrix(dist(latlong[,c("long","lat")]))
    }
    
    bw = bw # select bw using cross-val?
    W = gwr.Gauss(D,bw)
    W = ifelse(W==0,1.0e-300,W)
    dataGLM = merge(dataGLM,data.frame(ID=c(1:length(sts)), weight=W[j,]),by="ID")

    ######################################################################
    # Fit the model
    ######################################################################
    finalModel <- algo.farrington.glm(dataGLM,timeTrend=control$trend,populationOffset=control$populationOffset,
                                      factorsBool=control$factorsBool,weight=dataGLM$weight,reweight=control$reweight,
    ...
    if (is.null(finalModel)) {
      ...
      #Do we have an alarm -- i.e. is observation beyond CI??
      #upperbound only relevant if we can have an alarm (enoughCases)
      sts[[j]]@alarm[k] <- !is.na(X) && (X>1) && observed[k,j]!=0
      sts[[j]]@upperbound[k] <- ifelse(enoughCases,lu$upper,NA)
      ...
      # Get qAIC
      qAICs[(k-min(control$range)+1),j] <- finalModel$qAIC
  ...
  sts[[j]]@control$score <- score[,j]
  sts[[j]]@control$pvalue <- pvalue[,j]
  sts[[j]]@control$expected <- expected[,j]
  sts[[j]]@control$qAICs <- qAICs[,j]
  sts[[j]]@control$mu0Vector <- mu0Vector[,j]
  sts[[j]]@control$phiVector <- phiVector[,j]
  sts[[j]]@control$trendVector <- trendVector[,j]
  sts[[j]]@control$trend <- trend[,j]

  #Done
  return(sts[[j]][control$range,])
}
...
# FROM http://labbi.uesc.br/mirrors/cran/web/packages/bbmle/vignettes/quasi.pdf
################################################################################
...
algo.farrington.fitGLM.flexible <- function(dataGLM,weight,
  ...
  if (glmWarnings) {
    model <- glm(formula(theModel),weights = weight,data=dataGLM,family = quasipoisson(link="log"))
    model_qAIC = glm(formula(theModel),weights = weight,data=dataGLM,family = "x.quasipoisson")
  } else {
    model <- suppressWarnings(glm(formula(theModel),weights = weight,data=dataGLM,family = quasipoisson(link="log")))
    model_qAIC = suppressWarnings(glm(formula(theModel),weights = weight,data=dataGLM,family = "x.quasipoisson"))
  }
  ...
  #phi <- suppressWarnings(max(summary(model)$dispersion,1))
  phi = max(var(dataGLM[dataGLM$weight==1,'response'])/var(predict.glm(model,se.fit=TRUE, type="response")$fit[which(dataGLM$weight==1)]),1)
  ...
  model$qAIC = qAICc(model_qAIC,dispersion=phi,nobs=dim(dataGLM)[1])
  # if (reweight) {
  #   model$weights <- omega
  # } else{
...
algo.farrington.glm <- function(dataGLM,timeTrend,populationOffset,factorsBool,
                                weight,reweight,weightsThreshold,pThresholdTrend,b,
  ...
  finalModel <- list(pred,doTrend,coeffTime,model$phi,model$qAIC)
  names(finalModel) <- c("pred","doTrend","coeffTime","phi","qAIC")
  return(finalModel)
}
...
################################################################################
# Cross validation for bw
################################################################################
cv.bw = function(sts,formula,covariates,latlong,bw,control){
  show(paste("Bandwidth: ", bw,  sep = ""))

  fit <- geofarringtonFlexibleWithFormula(sts, formula,covariates,latlong,control = control,bw=bw)
  error = mean((fit@observed-attr(fit,'control')$expected)^2)
  show(paste("Error of cross validation: ", error,  sep = ""))

  return(error)
}

sel.bw = function(sts,formula,covariates,latlong,control,lower.bw = NULL, upper.bw = NULL){
 if(is.null(lower.bw) | is.null(upper.bw)){
   stop("Please input the bounds of bw.")  
 }

  opt <- optimize(cv.bw, lower = lower.bw, upper = upper.bw,
                  maximum = FALSE, sts=sts,formula=formula,covariates=covariates,latlong=latlong,control=control)
...
################################################################################
# Selection of bw based on qAIC
################################################################################
qAIC.bw = function(sts,formula,covariates,latlong,bw,control){
  show(paste("Bandwidth: ", bw, sep = ""))

  fit <- geofarringtonFlexibleWithFormula(sts, formula,covariates,latlong,control = control,bw=bw)
  score = mean(fit@control$qAICs)
  show(paste("Average qAICc: ", score, sep = ""))

  return(score)
}

select.bw = function(sts,formula,covariates,latlong,control,lower.bw = NULL, upper.bw = NULL){
 if(is.null(lower.bw) | is.null(upper.bw)){
   stop("Please input the bounds of bw.")  
 }

  opt <- optimize(qAIC.bw, lower = lower.bw, upper = upper.bw,
                  maximum = FALSE, sts=sts,formula=formula,covariates=covariates,latlong=latlong,control=control)
