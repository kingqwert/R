...
for(r in 1:3){
	eps=c(3,5,10)[r]
	result_each = matrix(NA,ncol=length(2:50),nrow=100)
	result_theta = matrix(NA,ncol=length(2:50),nrow=100)
	for(S in c(2:50)){
		K = S*eps
		if(K %%1 ==0){
			for(j in 1:100){
				...
				yi = numeric(K)
				for(i in 1:K){
					yi[i] = rnorm(1,mean=0,sd=(si[i]+tau2))
				}
				...
				theta_min = sub_res$TE.random+qnorm(0.05)/sqrt(sum(sub_res$w.random))
...
file = paste("/Users/daisuke/Dropbox/論文/Cherry picking/figs/Case1/Case1(theta0tau",tau2,").pdf",sep="")
...
file = paste("/Users/daisuke/Dropbox/論文/Cherry picking/figs/Case1/Case1_thetamin(theta0tau",tau2,").pdf",sep="")
g = ggplot(df_theta, aes(x = S, y = Mean, color=r))
g <- g + geom_line(position = pd,size=1) +geom_point(size = 1, shape = 18, position = pd) 
#g = g + geom_line()+geom_point(size = 2, shape = 16)+ylim(c(0,1))
#g <- g + geom_errorbar(aes(ymin = ifelse(Mean - 1.96*SD<0,0,Mean - 1.96*SD), ymax = ifelse(Mean + 1.96*SD>1,1,Mean + SD), width = 0.3),position = pd)+ scale_x_continuous(expand = c(0, 0),limits=c(0,16),breaks=c(0,2,5,10,15))
g <- g + geom_errorbar(aes(ymin =ifelse(Mean - SD<0,0,Mean-SD), ymax = Mean + SD, width = 0.3),position = pd)+ scale_x_continuous(expand = c(0, 0),limits=c(0,32),breaks=c(0,2,5,10,15,20,25,30))+ scale_y_continuous(limits=c(0,1.6),breaks=seq(0,1.6,0.2))
g = g+theme_classic()+theme(text=element_text(size=30,  family="serif"),legend.position="none")+ labs(x="",y="")
g
...
file = paste("/Users/daisuke/Dropbox/論文/Cherry picking/results/Case1_res(theta0tau",tau2,").RData",sep="")
save(res,file=file)
